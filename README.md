...
원본(`v2.7.1-beta`) 이후 이 포크에서 추가·개선된 항목들. 번호는 [`docs/CHANGES.md`](docs/CHANGES.md)의 섹션 번호와 일치합니다.

1. 앱 아이덴티티 분리 — `appId` / `productName` / 데이터 폴더를 모두 분리해서 원본 Freeter와 동시 설치·실행 가능.
2. Electron 36 → 41 업그레이드 (내장 Chromium 136 → 146) + `yarn dev` 크로스플랫폼 수정.
3. Webpage 위젯 링크·팝업 동작 개선 — 같은 창 이동과 새 창/팝업을 구분해서 처리 (13번에서 재정립).
4. Webpage 위젯에서 마우스 앞/뒤 버튼(X1/X2) 지원 — 커서 아래 webview를 대상으로 내비게이션.
5. File 메뉴에 **Open Data Folder** 추가 — 프로젝트/위젯 데이터 폴더를 OS 탐색기로 바로 열기.
6. **Ctrl+Tab / Ctrl+Shift+Tab** — 브라우저 탭처럼 같은 프로젝트 내 워크플로우 순환 전환.
7. User Agent 정비 — Google 계정 등 일부 서비스에서 "안전하지 않은 브라우저" 로그인 차단을 피하도록 UA 조정.
8. Note 위젯 공유 데이터 키 — 여러 워크플로우에서 같은 노트를 공유하는 cross-workflow 동기화.
9. TodoList 위젯 프로젝트 단위 자동 동기화 — 같은 프로젝트 내 워크플로우들이 할 일 목록을 자동 공유.
10. `useSharedDataChangedEffect` 훅 추상화 (내부 리팩토링) — 공유 데이터 구독 패턴 통일.
11. Webpage 위젯 동적 타이틀 — 현재 페이지 제목·URL을 위젯 헤더에 자동 반영.
12. Webpage 위젯 헤더 텍스트 선택·복사 허용 — 동적 타이틀(페이지 제목·URL)을 드래그로 블록 선택. 다른 위젯은 원본 동작 유지.
13. Webpage 위젯 링크 처리 재정립 — 새 탭 클릭은 기본 브라우저로, 팝업(OAuth 등)은 내부 창 유지.
14. Top Bar 높이 60 → 48px 축소로 워크스페이스 세로 공간 확보.
15. 워크플로우 탭·셸프 아이템 가로 폭 축소 (`min-width` 조정).
16. 새 위젯 기본 이름 공란 처리 — 자동 `Type 1`, `Type 2` 대신 바로 비워두고 헤더엔 위젯 타입 이름이 자연스럽게 노출.
17. 위젯 최소 크기 세로 1칸 허용 — Note(1×1), Webpage/TodoList(2×1), Timer(1×1) 등 납작한 배치 가능.
18. TodoList 항목 줄바꿈 — 긴 내용이 `…`로 잘리지 않고 여러 줄로 펼쳐져 전체 노출.
19. Shelf 위젯 호버 팝업 세로 절반 축소 — 300×300 → 300×150.
20. 기본 글로벌 단축키 `Ctrl/Cmd+Shift+Space`로 변경 — 원본 Freeter(`Ctrl/Cmd+Shift+F`)와 동시 실행 시 충돌 방지.
21. Link/File Opener 위젯 자동 아이콘 — URL의 favicon, 파일·폴더의 OS 네이티브 아이콘을 버튼에 자동 표시해서 여러 개 놔도 한눈에 구분. 실패 시 기존 기본 SVG로 자연 폴백.
22. Link/File Opener 위젯 동적 타이틀 — 첫 URL의 호스트(`github.com`) 또는 첫 경로의 파일명(`report.pdf`)을 헤더에 자동 반영. 여러 개면 `(+N)` 표시, 사용자가 이름을 지정했으면 그쪽이 우선.
23. Webpage 위젯 액션바에 **Copy URL** 버튼 추가 — 타이틀바 오른쪽 아이콘 줄에서 한 번 클릭으로 현재 페이지 주소 복사.
