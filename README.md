...
28. Webpage 위젯 액션바 버튼 툴팁에 단축키 표기 — 버튼에 마우스를 올리면 "Go Back (Alt+←)"처럼 바인딩된 키 조합을 함께 보여줘 액션바를 단축키 치트시트로 활용. 수정자는 OS별(Win/Linux `Ctrl`, macOS `Cmd`). *(2026-06-04)*
29. Webpage 위젯 새로고침 단축키 (`F5` / `Ctrl·Cmd+R`) — 위젯에 포커스가 있을 때 페이지를 새로고침하며, 브라우저처럼 현재 확대 배율은 유지. *(2026-06-04)*
30. 데이터 저장 안정성·정합성 개선 — 종료 직전 5초 내 변경이 사라지던 문제(종료 시 즉시 flush), 특수문자 키 저장 경로 불일치, 삭제 미완료/로드 실패 시 빈 화면 등 데이터 저장 경로의 결함을 정비. *(2026-06-04)*
31. File Explorer 위젯 추가 — 설정에서 등록한 즐겨찾기 폴더들만 트리 루트로(등록 순서 그대로) 띄우고(폴더 펼칠 때만 읽는 lazy 로딩), 더블클릭으로 OS 기본 앱·탐색기에서 열기 + 우클릭 Open/Copy Path. *(2026-06-05)*
32. File Explorer 위젯 개선 — 숨김 파일 토글(`.`로 시작하는 항목, 기본 꺼짐) 추가, 크기 표시를 끄면 파일별 `stat`을 건너뛰어 큰 폴더가 더 빠르게 열리고, 열다 실패한 폴더는 다시 펼치면 재시도. *(2026-06-06)*
33. (내부 성능) 공유 상태(shared state)를 쓰는 위젯(Note·File Opener)이 무관한 상태 변경에서도 매번 리렌더되던 문제를 2단계 동등성 비교로 제거 — 동작 변화 없이 헛 렌더만 감소. *(2026-06-06)*
34. 워크플로우 그리드를 2배 촘촘하게(16×8 → 32×16) — 위젯을 반칸 단위로 더 정밀하게 배치·리사이즈할 수 있고, 위젯 최소/기본 크기도 ×2로 맞춰 모양은 그대로. 칸 여백도 6→4px로 축소. (마이그레이션 없음: 기존 레이아웃은 한 번 수동 재조정 필요) *(2026-06-06)*
35. Webpage 위젯 — 액션바에 오디오 음소거 토글 버튼 추가(`setAudioMuted`). 음악·영상 사이트 위젯의 소리만 한 번에 끄고 켜기. *(2026-06-06)*
36. Webpage 위젯 — 페이지 내 찾기(Ctrl/Cmd+F 또는 돋보기 버튼). 임베드한 페이지에서 텍스트 검색·하이라이트, Enter/Shift+Enter로 다음/이전 이동. *(2026-06-06)*
37. File Explorer 위젯 — 우클릭 메뉴에 "Open containing folder"(파일의 상위 폴더 열기)와 "Copy name"(이름만 복사) 추가. *(2026-06-06)*
38. Webpage 위젯 — 페이지 로드 실패 시 빈 화면 대신 에러 오버레이 + Retry 버튼 표시(서브프레임·사용자 중단은 무시). *(2026-06-06)*
39. Note 위젯 — 공유 노트 동기화 개선: 포커스 중 도착한 원격 변경을 buffer해 blur 시 반영(누락 방지), 마크다운 모드도 `setContent`로 화면 갱신, 저장 디바운스 3초→800ms + 떠날 때 즉시 저장(지연 완화). *(2026-06-06)*
40. Note·To-Do List — 변경 직후 앱 종료/워크플로우 전환 시 디바운스 대기 중이던 마지막 변경이 유실되던 문제 수정(종료·언마운트 시 flush). *(2026-06-06)*
41. Webpage 위젯 다운로드 — 매번 저장 대화상자 대신 지정 폴더로 자동 저장(기본 OS Downloads, 충돌 시 `이름 (1).ext`). 앱 설정에서 폴더 변경 가능. *(2026-06-06)*
42. 워크플로우 배경 커스텀 — 앱 전역 설정에서 작업판 배경색·배경 이미지(채우기/맞춤/가운데/타일)·투명도 지정. 배경은 위젯 뒤 전용 레이어에 적용해 위젯 가독성 유지, 이미지는 main이 data URL로 안전하게 로드. *(2026-06-06)*
43. 리사이즈 중 위젯 투명도 0.5→0.8로 완화 — 크기 조절 중에도 위젯 내용이 또렷하게 보이도록(고스트 미리보기는 유지). *(2026-06-06)*
44. 위젯 모서리를 4px 살짝 둥글게 — 직각 테두리 대신 부드러운 라운드. *(2026-06-06)*
45. 워크플로우 바 위치 선택 — 위/아래/좌/우 배치(앱 설정). 좌/우면 세로 사이드 패널로 바뀌고 너비를 입력하거나 경계를 드래그해 조절. *(2026-06-06)*
46. Web Query 기본 엔진 확충 — Naver(검색·지도·쇼핑·주식·사전·뉴스)·Daum·카카오맵·YouTube·나무위키·ChatGPT·Claude·Perplexity·Google 번역/Scholar·Papago·알라딘·교보문고 등을 빌트인으로 추가(Custom Engine 없이 바로 선택). *(2026-06-06)*
47. Web Query 위젯에 검색창 여러 개 — 한 위젯에 엔진별 검색 입력을 세로로 쌓고, 설정에서 추가/삭제/순서이동. 기존 위젯은 자동 호환. *(2026-06-06)*
48. Top Bar 위젯 팝업 크기 조절 — worktable에서 올릴 때 그 크기로 시작하고, 에딧 모드에서 모서리 드래그로 조절(위젯별 영구 저장). *(2026-06-06)*
