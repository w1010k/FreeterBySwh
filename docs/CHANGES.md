...
- (후속) **같은 URL을 여러 탭에 등록하면 한 탭에서 페이지를 이동할 때 나머지 탭도 같이 이동하는 버그**. 탭들이 세션 파티션을 공유하다 보니 동일 origin 게스트끼리 SharedWorker/BroadcastChannel을 공유하고, Notion처럼 이를 써서 탭 간 상태를 동기화하는 앱이 문자 그대로 함께 움직였다. 우리 렌더러 코드와 main의 단축키 라우팅은 전부 `webContentsId` 단위라 용의선상에서 제외됐고, 남는 레버는 파티션뿐. **중복 URL 탭에만** 파티션 접미사(`:dup1`, `:dup2`…)를 붙여 세션을 분리했다 — 첫 등장 탭의 파티션 문자열은 한 글자도 바뀌지 않으므로 중복이 없는 기존 설정은 완전 무영향이고, 대가는 중복 탭이 별도 세션이라 로그인을 따로 해야 한다는 점(의도된 트레이드오프: 같은 URL을 두 번 등록했다는 건 따로 보겠다는 뜻). 함께 `tabInfos` 키를 URL에서 탭 키로 바꿔, 중복 탭이 서로의 타이틀·파비콘·음소거 아이콘을 덮어쓰던 문제도 해결.
