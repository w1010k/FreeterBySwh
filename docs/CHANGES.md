...
## 17. Note 위젯 최소 크기 축소 (2×2 → 1×1)

좁은 공간에 짧은 메모만 붙여두고 싶은 경우가 많은데 `minSize: { w: 2, h: 2 }` 때문에 과도하게 자리 차지. 이미 1×1로 동작하는 위젯(commander, file-opener, link-opener)이 있어서 그리드 시스템 상 안전하게 내릴 수 있음. 실제 입력 가능한 최소치는 레이아웃 그리드 단위 1×1.

**수정 파일**: `src/renderer/widgets/note/index.ts`
