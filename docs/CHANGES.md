...
[#8](#8-note-위젯-공유-데이터-키-cross-workflow-sync) 같은 키를 공유하는 노트끼리 "간헐적으로 동기화가 안 된다"는 문제의 **수신 측 원인 두 가지**를 고쳤다. (마크다운 모드의 별도 이슈는 아래 한계 참고)
...
- **한계(마크다운 모드)**: 마크다운을 켜면 보이는 편집기가 tinyMDE(별도 요소)라 textarea의 blur가 안 뜨고, 외부 변경이 tinyMDE 화면에 반영되지 않는다 — 이건 별도 이슈(N2)로 남겨 추후 처리. 디바운스 단축(N3)은 마크다운에도 적용됨.

### 수정 파일

- **수정**: `src/renderer/widgets/note/widget.tsx`(보류-리로드, blur flush, 800ms 디바운스)
- **테스트**: `tests/renderer/widgets/note/widget.spec.ts`(800ms 디바운스·blur flush·포커스 중 보류 후 blur 반영)
